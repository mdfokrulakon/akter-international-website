...
├── logo.svg            # Placeholder logo — replace with the official logo
...
`logo.svg` in this project is a **placeholder wordmark** (no official logo file was supplied). To use the real Akter International logo:

1. Save the official logo file as `logo.svg` in the project root, replacing the placeholder.
2. Keep the filename and location the same — `index.html` already references `logo.svg` in the header, footer, and favicon link, so no other changes are needed.
3. If your logo is a raster file (PNG/JPG) instead of SVG, either convert it to SVG, or update the `href`/`src` attributes in `index.html` to point at the new filename (e.g. `logo.png`).
